...
```{python}
pa["sedentary_ap_m_wk"] = (pa["sedentary_ap_s_day"] / 60 * 7).round(1)
pa[["sedentary_ap_m_wk", "sedentary_ap_s_day"]]
```

```{python}
pa["light_ap_m_wk"] = (pa["light_ap_s_day"] / 60 * 7).round(1)
pa[["light_ap_m_wk", "light_ap_s_day"]]
```

```{python}
pa["mvpa_m_wk"] = (pa["mvpa_s_day"] / 60 * 7).round(1)
pa[["mvpa_m_wk", "mvpa_s_day"]]
```

```{python}
pa[["sedentary_ap_s_day", "light_ap_s_day", "mvpa_s_day"]]
```
...
pa[["light_ap_m_wk", "mvpa_m_wk", "total_pa_m_wk"]]
```
...
```{python}
def physical_classify(total_pa):
    if total_pa < 500:
        return "low"
...
```{python}
physical_classify_vec = np.vectorize(physical_classify)
```

```{python}
pa["total_pa_cat"] = physical_classify_vec(pa["total_pa_m_wk"])
pa[["total_pa_m_wk", "total_pa_cat"]]
```
...
```{python}
px.box(pa, y = "total_pa_m_wk",
color = "sex",
labels = {"total_pa_m_wk" : "Minutes per Week", "sex": "Sex"},
title = "Total Physical Activiy by Sex")
```

Write a short explanation of what you see in the plot.

Apparently, male persons, on average, reported doing more physical activity than female persons. There are a few outliers in both sexes, as well. 