...
yra_exp_vec = np.vectorize(yrs_exp)

# Apply the function to the dataset
jobs["years_required"] = yra_exp_vec(jobs["job_description"])
jobs
```
...
```{python}
jobs.to_csv("outputs/jobs_description_years_experience.csv", index = False)
```
...
```{python}
jobs["years_required_num"] = pd.to_numeric(jobs["years_required"], errors = "coerce")
jobs
```
...
```{python}
jobs['state'] = jobs['location'].str.extract(r',\s*(\w{2})')
jobs
```
...
```{python}
px.scatter(jobs, x = "years_required_num",
           y = "midpoint_salary_estimate",
           color = "state",
           labels = {"midpoint_salary_estimate" : "Midpoint Salary Estimate (USD)",
                     "years_required_num" : "Experience Required (in years)"},
           title = "Relationship between Midpoint Salary Estimated and Years of Experience")
```

Describe any relationships you see in the plot.

I do not see any clear relationship between years of ecperience and midpoint salary estimate, nevertheless, some few years of expericne requierd and very high midpoint salary estimate jobs offerings are observed in the state of California (red points).
...
def programming_language(description):
    prompt = f"""
    Please, extract the programming language required in the job description (R (it could appear as R, RStudio, rstudio or just r), Python, Both, or Niether). 
    Return ONLY the name of the programming language requireda (R and/or Python).
    In case both languages are required, return the word "Both".
    If no programming language is required, return the word "Neither".
    Here's the job description:
    {description}
    """
    return llm_chat(prompt)
...
```{python}
px.box(jobs, x = "lang_req", y = "midpoint_salary_estimate", labels = {"midpoint_salary_estimate" : "Midpoint Salary Estimate (USD)", "lang_req" : "Programming Language Required"})
```
...
According to the results returned by our LLM fuction, no job description required the R programming language by itself. As a matter of fact, the majority of the job descriptions required neither programming language skills, 17 job descriptions required both programming languages, and just 12 required Python programming language. Regarding midpoint salary estimate, no big differences are observed, nevertheless, some interesting outliers regarding Python are detected, showing that if someone learns this programming language may be lucky to get a big salary. 
...
```{python}
# Define the function here
def technical_skill(description):
    prompt = f"""
    Please, extract the basic four technical skills required in the job description (proficiency in programming languages, computer operation, problem-solving techniques, and effective communication within technical contexts). 
    Please, return ONLY the name of the technical skill requireda (proficiency_programming_languages, computer_operation, problem_solving_techniques, and effective_communication).
    In case two or more of these skills are required, return the word "Two_or_more".
    If no technical skill is required, return the word "NaN".
    Here's the job description:
    {description}
    """
    return llm_chat(prompt)
# Vectorize the function
technical_skill_vec = np.vectorize(technical_skill)
# Apply the function to the dataset
jobs["tech_skill"] = technical_skill_vec(jobs["job_description"])
jobs
```

```{python}
jobs["tech_skill"].value_counts()
```