...
Use the [] operators to select the weight_kg and height_cm variables in the yao dataframe. Assign the results to a new dataframe called yao_weight_height
...
Asign the results to a new dataframe yao_no_education_consultation
...
```{python}
yao.filter(regex="result$") # end with result
```
...
Rename the age_category in the yao dataframe to age_cat. Assign the result to a new dataframe called yao_age_cat.
...
```{python}
cleanin_practice.columns = cleanin_practice.columns.str.replace(r"[^a-zA-Z0-9]", "_", regex=True)
cleanin_practice
```
