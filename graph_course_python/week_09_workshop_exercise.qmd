...
```{python}
population_total = population[(population['indicator'] == "SP.POP.TOTL")]

population_total
```

```{python}
population_total_long = pd.melt(population_total, id_vars = ['country', 'indicator'], var_name = 'year', value_name = 'population')

population_total_long
```

```{python}
population_total_long = population_total_long.rename(columns = {"country": "country_code"})

population_total_long
```

```{python}
population_long = population_total_long.drop(columns = ["indicator"])
...
hiv_data_combined_pop = pd.merge(hiv_data_combined, population_long, on = ['country_code', 'year'], how =
'inner')
...
```{python}
# Calculate incidence and prevalence rate
hiv_data_combined_pop["incidence_rate"] = (
    100000 * hiv_data_combined_pop["new_cases"] / hiv_data_combined_pop["population"]
)

hiv_data_combined_pop["prevalence_rate"] = (
    100000 * hiv_data_combined_pop["total_cases"] / hiv_data_combined_pop["population"]
)
hiv_data_combined_pop
```
...
```{python}
unique_categories = hiv_data_combined_pop['country'].unique()
...
hiv_data_mini = hiv_data_combined_pop[
    (hiv_data_combined_pop['country'] == "Guatemala") | (hiv_data_combined_pop['country'] == "El Salvador") | (hiv_data_combined_pop['country'] == "Honduras")]
...
```{python}
# Your code here
px.line(hiv_data_mini, x = "year", y = "incidence_rate", color = "country", markers = True)
```

```{python}
# Your code here
hiv_data_mini_2 = hiv_data_combined_pop[
    (hiv_data_combined_pop['country'] == "Brazil") | (hiv_data_combined_pop['country'] == "Argentina") | (hiv_data_combined_pop['country'] == "Chile")]

hiv_data_mini_2['year'] = hiv_data_mini_2['year'].astype(int)
...
```{python}
# Your code here
px.line(hiv_data_mini_2, x = "year", y = "incidence_rate", color = "country", markers = True)
```
...
```{python}
# Your code here
px.line(hiv_data_mini, x = "year", y = "prevalence_rate", color = "country", markers = True)
```

```{python}
# Your code here
px.line(hiv_data_mini_2, x = "year", y = "prevalence_rate", color = "country", markers = True)
```
