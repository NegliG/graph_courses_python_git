...
demog_loan_info = loans.filter(items=keep_columns).rename(
...
high_income.to_csv("outputs/high_risk.csv", index=False)
```
...
with pd.ExcelWriter('outputs/loans_subsets.xlsx') as writer:
    demog_loan_info.to_excel(writer, sheet_name='Demographic Loan Info', index=False)
    credit_vars.to_excel(writer, sheet_name='Credit Vars', index=False)
