...
The highest mean total cost of tretment is reported by the Gradute level of education. 
...
```{python}
px.histogram(edu_cost_table, x = "education", y = "mean_total_cost")
```

Write a short interpretation of the plot.

Apparently, as higher the education level, higher the mean total cost paid by the patients. 
...
```{python}
px.histogram(age_cost_table, x = "age_group", y = "mean_total_cost")
```

Write a short interpretation of the plot.

The mean total cost is higher at extreme ages. It goes higher between the 18-36 age group, then it falls for both the 37-47 and 48-57 age groups, and finally goes up again for the 58+ age group.
...
```{python}
# Create summary table for tratment initiation delay in days
treatment_initiation_delay_in_days = pd.DataFrame(
   tb.groupby("first_visit_type")
    .agg(
        count_patients = ("id", "size"),
        total_treatment_delay_days = ("treatment_initiation_delay_(in_days)", "sum"),
        mean_total_treatment_delay_days = ("treatment_initiation_delay_(in_days)", "mean")
    )
    .reset_index()
)
